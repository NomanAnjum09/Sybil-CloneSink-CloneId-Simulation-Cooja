...
create_beacon(void)
{
  uint8_t neigh = get_neighbour_address();
  ...
  if(p->header.dst.u8[1]!=neigh){
  printf("Forged %d as source instead %d\n",neigh,p->header.src.u8[1]);
  printf("Sending forged Forecast \n");
  p->header.src.u8[1] = neigh;
  print_packet(p);}
  return p;
