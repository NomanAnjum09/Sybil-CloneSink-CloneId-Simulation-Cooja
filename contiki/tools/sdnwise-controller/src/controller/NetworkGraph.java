...
public class NetworkGraph {
	...
	ConcurrentHashMap<String,String> neighborTableRSSI = new ConcurrentHashMap<String, String>();
...
//**************Sybil Detection*************
	public void check_sybil1(String[] packet) {
		String SENDER = packet[3].substring(packet[3].length() - 1);
		String neighbours="";
		System.out.println("Checking Sybil for "+SENDER);
		for(int i = 11;i+1<packet.length;i+=3) //display neighbor table
 		{ 
 			String rssi = packet[i+1];
 			String negh = packet[i];
 			
 			if(neighborTableRSSI.containsKey(negh)) {
 				System.out.println("Checking neighbour" + negh+ " Of "+ SENDER);
 				String all[] = (neighborTableRSSI.get(negh).split(","));
 				for(int j=0;j+1<all.length;j+=2) {
 					String curr = all[j];
 					String currRSSI = all[j+1];
 					System.out.println(curr+": "+currRSSI+" ,"+SENDER+": "+rssi);
 			...
 			neighbours=neighbours+negh;
 			neighbours=neighbours+",";
 			neighbours=neighbours+rssi;
 			neighbours=neighbours+",";
 			}
		
		System.out.println(neighbours);
		if(neighborTableRSSI.containsKey(SENDER)) {
			neighborTableRSSI.replace(SENDER, neighbours);

		}
		else
		neighborTableRSSI.put(SENDER, neighbours);
